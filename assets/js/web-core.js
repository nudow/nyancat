...
W.Classes.newClass({name:"wysiwyg.viewer.managers.TPAManager",imports:["wysiwyg.viewer.managers.tpa.URLBuilder","wysiwyg.viewer.managers.tpa.TPAComponentRegistrar"],Class:{Extends:"mobile.core.managers.BaseManager",Binds:["_onAppAvailableTimeoutExpires"],Static:{TPA_MESSAGE:"TPA",TPA_MESSAGE2:"TPA2",TPA_RESPONSE:"TPA_RESPONSE",APP_MARKET_MESSAGE:"APP_MARKET",PINGPONG_PREFIX:"pingpong:",LOAD_TIMEOUT:20,MessageTypes:{CHANGE_APP_SIZE:"changeAppSize",RESIZE_WINDOW:"resizeWindow",REFRESH_APP:"refreshApp",APP_IS_ALIVE:"appIsAlive",APP_STATE_CHANGED:"appStateChanged",CLOSE_POPUP:"closePopup",OPEN_POPUP:"openPopup",OPEN_MODAL:"openModal",OPEN_MEDIA_DIALOG:"openMediaDialog",OPEN_FULLSCREEN:"openFullscreen",OPEN_BILLING_PAGE:"openBillingPage",SM_REQUEST_LOGIN:"pingpong:smRequestLogin",SM_CURRENT_MEMBER:"pingpong:smCurrentMember",SITE_INFO:"pingpong:siteInfo",EVENT_LISTENER_ADDED:"pingpong:addEventListener",HEIGHT_CHANGED:"heightChanged",APP_SETTINGS_CHANGED:"appSettingsChanged",APP_SETTINGS_CLOSE:"appSettingsClose",APP_SHOW_POPUP:"appShowPopup"}},callId:1,callbacks:{},initialize:function(){this._urlBuilder=new this.imports.URLBuilder();
...
}else{if(a.intent===this.TPA_MESSAGE2&&b){this._handleTPAMessage2(a,b,f)
}}}}},_handleTPAMessage2:function(a,b,g){var d=a.compId;
var c=g(d);
if(!c){return
}if(Object.contains(this.MessageTypes,b)){var e=this._getHandlerFunctionName(b);
var f=c[e];
if(f){f.apply(c,[{msgData:a.data,callback:function(i){var h=c.getIFrame();
i.staticUrl=W.Config.getMediaStaticUrl(i.uri)+i.uri;
h.contentWindow.postMessage(JSON.stringify({intent:this.TPA_RESPONSE,compId:d,callId:a.callId,type:b,res:i,status:true}),"*")
}.bind(this)}])
